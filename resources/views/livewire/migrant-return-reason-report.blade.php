                ...
                <table class="table-auto text-[13px] text-center border-collapse border border-gray-300 w-full">
                    <thead class="bg-gray-100">
                        <tr>
                            ...
                            <th class="border border-gray-300 py-1">फर्किनुको कारण</th>
                    ...
                    <tbody>
                        @foreach ($returnReasonData as $returnReason => $data)
                            <tr>
                                <td class="border border-gray-300 py-1">{{ $loop->iteration }}</td>
                                <td class="border border-gray-300 py-1">{{ $returnReason }}</td>
                                <td class="border border-gray-300 py-1">{{ $data }}</td>
                                <td class="border border-gray-300 py-1">{{ $returnReasonPercentage[$returnReason] }}
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
                    <tfoot>
                        <tr>
                            ...
                            <th class="border border-gray-300 py-1">{{ array_sum($returnReasonData) }}</th>
                            <th class="border border-gray-300 py-1">
                                {{ array_sum($returnReasonPercentage) }}</th>
                        </tr>
                    </tfoot>
                </table>
            </div>
...
@script
    <script>
        ...
        new Chart(ctx, {
            ...
            data: {
                datasets: [{
                    label: 'फर्किनुको कारण',
                    data: @json($this->returnReasonData),
                    borderWidth: 1
                }, ]
            },
            options: {
        ...
        new Chart(ctx2, {
            type: 'pie',
            data: {
                labels: Object.keys(@json($this->returnReasonPercentage)),
                datasets: [{
                    label: 'जनसंख्या',
                    data: Object.values(@json($this->returnReasonPercentage)),
                    borderWidth: 1,
                    hoverOffset: 4
                }],
